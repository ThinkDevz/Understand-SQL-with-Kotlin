...
class Main2Activity : AppCompatActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		if (info.equals("new")) {
			val background = BitmapFactory.decodeResource(applicationContext.resources, R.drawable.client)
			select.setImageBitmap(background)
	...
	override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
		if (requestCode== 1 && resultCode == Activity.RESULT_OK && data != null){
			...
			try {
				selectedImage = MediaStore.Images.Media.getBitmap(this.contentResolver, images)
				select.setImageBitmap(selectedImage)
	...
	fun save(view: View){
		...
		startActivity(Intent(applicationContext, ArtBook::class.java))
